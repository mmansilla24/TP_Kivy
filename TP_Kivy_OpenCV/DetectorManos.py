...
class CameraScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        boton_indice = Button(text="<- Atrás", size_hint=(None, None), size = (100, 50))
                        ...
                        if mano == 'Right':
                            if self.cont_der >= 5:
                                ...
                                if not self.is_recording:
                                    self.grabar_video = cv2.VideoWriter(f"fotos_videos/video{date_time_str}.mp4", cv2.VideoWriter_fourcc(*'XVID'), 20.0, (self.frame_width, self.frame_height))
